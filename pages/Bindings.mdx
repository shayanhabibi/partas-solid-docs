...
## PartasProxyImportAttribute

If you're binding a library that provides composed components via a proxy such as from solid-motion, then you'll want to make use of the
`PartasProxyImport`. It is much the same as the normal Fable `Import`, except it takes an additional parameter as the key to the proxy
which is appended to the end of the selector:

```fsharp
[<Erase; RequireQualifiedAccess>]
module Motion =
    [<PartasProxyImport("div","Motion","solid-motion")>]
    type div() =
        inherit div()
